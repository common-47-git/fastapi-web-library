...
async def authors_all(session: async_session_dependency):
    authors = await crud.read_entities(
        alchemy_model=AuthorsModel, session=session,
    )
    if not authors:
...
async def authors_add(
    ...
    return await crud.create_entity(
        alchemy_model=AuthorsModel, pydantic_schema=author, session=session,
    )


@router.delete(
    "/delete/{author_id}", response_model=authors_schemas.AuthorDelete,
)
