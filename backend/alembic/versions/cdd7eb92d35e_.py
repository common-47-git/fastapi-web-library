...
Revision ID: cdd7eb92d35e
Revises: 
Create Date: 2025-06-19 23:53:14.353713
...
def upgrade() -> None:
    ...
    sa.Enum("Translated", "In progress", "Abandoned", "Absent", name="translation_status_enum").create(op.get_bind())
    sa.Enum("Reading", "Have read", "TO read", "Favorites", name="book_shelf_enum").create(op.get_bind())
    ...
    sa.Column("book_translation_status", postgresql.ENUM("Translated", "In progress", "Abandoned", "Absent", name="translation_status_enum", create_type=False), nullable=True),
    ...
    sa.Column("book_id", sa.UUID(), nullable=False),
    sa.Column("user_id", sa.UUID(), nullable=False),
    sa.Column("book_shelf", postgresql.ENUM("Reading", "Have read", "TO read", "Favorites", name="book_shelf_enum", create_type=False), nullable=True),
    sa.ForeignKeyConstraint(["book_id"], ["books.book_id"] ),
    ...
    sa.UniqueConstraint("volume_number"),
    )
    ...
    sa.ForeignKeyConstraint(["volume_id"], ["volumes.volume_id"] ),
...
def downgrade() -> None:
    ...
    sa.Enum("Reading", "Have read", "TO read", "Favorites", name="book_shelf_enum").drop(op.get_bind())
    sa.Enum("Translated", "In progress", "Abandoned", "Absent", name="translation_status_enum").drop(op.get_bind())
    # ### end Alembic commands ###
